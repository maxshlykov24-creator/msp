...
def pairs() -> list[tuple[str, str]]:
    ...
    for old_s, new_s in RAZUM_3DOT_TO_DD_DOT:
        ...
        out.append((f"Личное/1. Профиль/3. Разум/{old_s}", f"Личное/1. Профиль/3. Разум/{new_s}"))
        out.append((f"1. Профиль/3. Разум/{old_s}", f"1. Профиль/3. Разум/{new_s}"))
        out.append((f"3. Разум/{old_s}", f"3. Разум/{new_s}"))
        out.append((f"3.%20Разум/{oe}", f"3.%20Разум/{ne}"))

    for old_s, new_s in RAZUM_OLD_NEW:
        oe, ne = old_s.replace(" ", "%20"), new_s.replace(" ", "%20")
        out.append((f"Личное/2. Профиль/3. Разум/{old_s}", f"Личное/1. Профиль/3. Разум/{new_s}"))
        out.append((f"2. Профиль/3. Разум/{old_s}", f"1. Профиль/3. Разум/{new_s}"))
        out.append((f"3. Разум/{old_s}", f"3. Разум/{new_s}"))
        out.append((f"3.%20Разум/{oe}", f"3.%20Разум/{ne}"))

    out.append(("Личное/2. Профиль", "Личное/1. Профиль"))
    out.append(("Личное/3. Тесты", "Личное/2. Тесты"))
    out.append(("Личное/1. Входящее", "Личное/0. Входящее"))
    ...
    out.append(("Личное/00_Входящее", "Личное/0. Входящее"))
    out.append(("Личное/01_Профиль", "Личное/1. Профиль"))
    out.append(("Личное/02_Тесты", "Личное/2. Тесты"))
    out.append(("01_Профиль/", "1. Профиль/"))
    out.append(("01_Профиль", "1. Профиль"))
    out.append(("00_Входящее", "0. Входящее"))
...
def patch_text(s: str) -> str:
    ...
    s = s.replace("../00_Входящее", "../0. Входящее")
    s = s.replace("../2. Профиль/", "../1. Профиль/")
    s = s.replace("../1. Входящее", "../0. Входящее")
    s = re.sub(r"`01_Профиль`", "`1. Профиль`", s)
    s = re.sub(r"# 01_Профиль\b", "# 1. Профиль", s)
    s = re.sub(r"`Личное/01_Профиль`", "`Личное/1. Профиль`", s)

    # Оставшиеся вхождения «корень личного» без префикса Личное/
    s = re.sub(r"(?<!\d)1\. Входящее\b", "0. Входящее", s)
