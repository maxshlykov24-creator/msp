...
Локальный vault: `/Users/max/Desktop/CURSOR`  
Ветка для sync: **`main` только**

## Быстрый старт (один раз)

```bash
cd /Users/max/Desktop/CURSOR
...
```bash
# Пауза авто-sync
touch /Users/max/Desktop/CURSOR/.vault-sync-off
...
2. В терминале (если доступна CLI `agent`):
   ```bash
   agent worker start --worker-dir /Users/max/Desktop/CURSOR
   ```
...
Vault на `~/Desktop/CURSOR`. Если включена синхронизация **Desktop & Documents** в iCloud — git + iCloud могут конфликтовать.  
