...
async def cmd_start(message: Message) -> None:
    ...
    try:
        already_bound = sess.execute(
            select(ContactBinding).where(ContactBinding.telegram_chat_id == chat_id)
        ).scalar_one_or_none()
    finally:
        sess.close()
...
def _gather_status_lines_for_chat(chat_id: int) -> list[tuple[str, str]] | None:
    ...
    try:
        ...
        if not binding:
            return None
        leads = amocrm_client.find_leads_by_contact_id(sess, int(binding.amo_contact_id))
        cdek_fid = int(s.amo_field_lead_cdek or 0)
        st_fid = int(s.amo_field_lead_liveinform_status or 0)
        items: list[tuple[str, str]] = []
        for lead in leads:
            track = amocrm_client.extract_lead_field_value(lead, cdek_fid) if cdek_fid else ""
            status = amocrm_client.extract_lead_field_value(lead, st_fid) if st_fid else ""
            if track or status:
                items.append((track, status))
        return items
    finally:
        sess.close()
