...
async def _read_liveinform_payload(request: Request) -> dict[str, Any]:
    """LiveInform шлёт application/x-www-form-urlencoded c полем data=<json>.
    Поддерживаем также чистый JSON в теле для обратной совместимости.
    Внимание: после `request.body()` поток исчерпан, поэтому форму парсим вручную из bytes.
    """
...
async def lifespan(_: FastAPI):
    init_db()
    s = get_settings()
    polling: Optional[asyncio.Task[None]] = None
    if (s.telegram_bot_token or "").strip():
        from app.bot import run_bot_forever

        polling = asyncio.create_task(run_bot_forever())
