...
Звонки и минуты (`call_*` + `duration`), новые лиды, конверсия лид→сделка
(Не обработан→Успех), повторные продажи, выставленные счета и приход
(`price` на стадиях «Отправлен счет»/«Успех»), средний чек, сообщения
...
Оговорки: ~30% выигранных сделок с `price=0` (гигиена данных на стороне
клиента); привязка сообщений/времени ответа к менеджеру — по
`responsible_user_id` (Wazzup ставит `created_by=0`).
...
- **Потоковые** (выручка, лиды, конверсия, звонки, минуты, сообщения вх/исх,
  ...
  (`cohort_won/new_leads`, по дате создания лида).
...
за 62 дня + события за последние дни. Историю на 12 мес наполняет разовый
...
| `GET /api/funnels\|stuck\|loss_reasons\|meta` | состояние «на момент» | да |
