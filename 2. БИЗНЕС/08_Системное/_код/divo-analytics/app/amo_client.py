...
class AmoClient:
    ...
    def _patch(self, path: str, json_body: Any) -> dict[str, Any] | None:
        self._limiter.wait()
        try:
            resp = self._client.patch(path, json=json_body)
        except httpx.TransportError as exc:
            log.warning("amo transport error on PATCH %s: %s", path, exc)
            raise

        if resp.status_code == 204:
            return None
        if resp.status_code == 429 or resp.status_code >= 500:
            log.warning("amo PATCH %s -> %s, retry", path, resp.status_code)
            raise AmoRetryable(f"{resp.status_code} on PATCH {path}")
        if resp.status_code == 401:
            raise AmoError("amo 401: токен недействителен или истёк")
        if resp.status_code >= 400:
            raise AmoError(f"amo {resp.status_code} on PATCH {path}: {resp.text[:300]}")
        return resp.json()
    ...
    def patch_batch(self, path: str, items: list[dict[str, Any]]) -> dict[str, Any] | None:
