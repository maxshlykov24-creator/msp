#!/usr/bin/env bash
# Nginx vhost + Let's Encrypt для divomotors-analytics.ru → 127.0.0.1:19100.
...
FQDN="${FQDN:?Укажите: export FQDN=divomotors-analytics.ru}"
...
CFG_SRC="${SCRIPT_DIR}/nginx/divo-analytics.conf"
[[ -f "$CFG_SRC" ]] || { echo "Не найден ${CFG_SRC}" >&2; exit 1; }

sed "s/__FQDN__/${FQDN}/g" "$CFG_SRC" > /etc/nginx/sites-available/divo-analytics
ln -sf /etc/nginx/sites-available/divo-analytics /etc/nginx/sites-enabled/divo-analytics
