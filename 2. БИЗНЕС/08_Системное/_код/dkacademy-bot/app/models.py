...
class ContactBinding(Base):
    ...
    # Используется как универсальный ID адресата: chat_id для Telegram, user_id для MAX
    telegram_chat_id: Mapped[str] = mapped_column(String(64), index=True)
    channel: Mapped[str] = mapped_column(String(16), default="telegram", server_default="telegram", index=True)
    ...
    __table_args__ = (
        UniqueConstraint("channel", "telegram_chat_id", name="uq_channel_chat_id"),
    )
