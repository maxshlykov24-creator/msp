...
## Автоназначение ответственного («Повторные продажи»)

Когда сделка контакта **создаётся** или **переходит** на этап «Требует
касания» или «Не обработан» в воронке «Повторные продажи» — ответственным
автоматически ставится тот, кто закрыл **последнюю** успешную сделку этого
контакта в «Продажи» **или** «Повторные продажи» (по `closed_at`, берём самую
свежую из двух воронок). Тот же пользователь ставится ответственным и на
контакте. Если успехов нет ни в одной из воронок (или ответственный там —
служебный аккаунт Максим / Максим Дейкало) — берём ответственного с карточки
контакта и ставим его на сделку. Служебный аккаунт источником не считаем.
Срабатывает на **каждый** вход в эти этапы (даже если ответственного до этого
меняли руками).

Источник события — веб-хук amoCRM (в отличие от «Требует касания» выше, эта
логика не привязана к тому, кто создал сделку — бот, менеджер вручную или
клиент, написавший сам и попавший в «Не обработан»).

**Настройка (.env):**

```
AMO_STATUS_NE_OBRABOTAN=      # id этапа «Не обработан» в «Повторные продажи» (см. дискавери)
AMO_WEBHOOK_SECRET=            # секрет в пути POST /webhooks/amocrm/{token}
```

Этап «Требует касания» и воронки берутся из настроек выше (`AMO_PIPELINE_SALES`,
`AMO_PIPELINE_REPEAT`, `AMO_STATUS_WON`, `AMO_STATUS_TREBUET_KASANIYA`).

**Регистрация в amoCRM:** `Настройки → Веб-хуки` (или `POST /api/v4/webhooks`) —
события **«Добавление»** и **«Смена статуса»** сделки, URL:
`https://<домен dkacademy-bot>/webhooks/amocrm/<AMO_WEBHOOK_SECRET>`.
...
| POST | `/webhooks/amocrm/{token}` | Вход amoCRM (автоназначение ответственного в «Повторные продажи») |
