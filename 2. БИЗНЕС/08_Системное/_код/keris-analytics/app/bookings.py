...
def fetch_bookings() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Все записи и мастера. Нет DATABASE_URL — пустые списки, питомник всё равно собирается."""
    ...
    with psycopg.connect(dsn) as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """
                SELECT id, owner_phone, master_id, starts_at, price, status, created_at
                FROM bookings
                """
            )
