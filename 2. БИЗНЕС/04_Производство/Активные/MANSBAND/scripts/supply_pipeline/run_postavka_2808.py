#!/usr/bin/env python3
"""Пайплайн Поставка 28.08 → flat / Прогон / Блокеры / (опц.) МС."""
...
DEFAULT_XLSX = (
    ...
    / "поставки"
    / "2026-08-28_Поставка-для-внесения.xlsx"
)

config.set_run(
    flat="Поставка 28.08 flat",
    run="Прогон 28.08",
    block="Блокеры 28.08",
    external_code="postavka-2026-08-28",
    description="Поставка 28.08",
    moment="2026-08-28 12:00:00.000",
)

import run_postavka_1507 as runner  # noqa: E402

# Бренд Mansband уже в справочнике → M. Новые только модели (как IM7PR22TR5D6).
NEW_BRANDS: list = []
NEW_MODELS = [
    ("X1PR22TR6D6", "X1PR22TR6D6"),
    ("X1PANTD6", "X1PANTD6"),
]
...
def ensure_spr_abbrs(svc) -> None:
    """Дописать Mansband / X1* в Справочник, если нет."""
    ...
    for name, abbr in NEW_BRANDS:
        ...
        # чтобы следующий empty не попал в ту же строку
        while len(spr) < row:
            spr.append([])
        r = list(spr[row - 1]) + [""] * 14
        r[10], r[11] = name, abbr
