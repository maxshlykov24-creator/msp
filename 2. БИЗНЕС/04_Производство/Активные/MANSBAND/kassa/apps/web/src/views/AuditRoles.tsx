...
import { History, Search, UserCog, UserPlus, Check } from "lucide-react";
...
const MOCK_USERS: RoleUser[] = [
  { id: "u1", name: "Матвей", login: "matvey", role: "seller", active: true },
...
export function RolesScreen() {
  ...
  async function createUser() {
    ...
    if (newPassword.length < 8) return setCreateError("Пароль: минимум 8 символов");
    setCreating(true);
    try {
      if (!USE_MOCK) {
        const created = await api.post<RoleUser>("/users", {
          name: newName.trim(),
          login: newLogin.trim(),
  ...
  useEffect(() => {
    ...
    if (canManage) {
      ...
      setUsers([{ id: user.id, name: user.name, login: user.login, role: user.role, active: true }]);
    }
  }, [canManage, user]);
            ...
            <tbody className="divide-y divide-ink-800">
              {users.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="px-4">
                    <span className="chip bg-white text-ink-950">
                      {row.active === false ? "Отключён" : "Активен"}
                    </span>
                  </td>
                </tr>
              ))}
